...
このリポジトリでは、原則として後方互換性を維持しない。設計や責務分割を改善する場合は、
過去の API、import パス、CLI、出力形式などを残すより、現在の正規の形へ一括して揃える。
互換ラッパー、re-export alias、deprecated shim、新旧両対応の分岐は追加しない。

変更時は、実装、テスト、文書、examples、`requirements.py` などのリポジトリ内参照を同時に更新する。
後方互換性を維持するのは、対象 Issue にその理由と維持範囲が明記されている場合に限る。
これは理由のない破壊的変更を勧める方針ではなく、変更後の設計を一貫させ、不要な旧仕様を残さないための方針である。
