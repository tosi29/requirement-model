...
- 種別: QualityRequirement / 状態: verified / 優先度: 2
- 根拠:
...
- **Goal / Need は帯 (枠) にまとめ、常に図の上に表示**する
...
**型は色と形、status は線種、優先度は輪**で表す。3 つの軸が互いを潰さないように
割り当ててあるので、影響範囲を色で追いながら同時に status と優先度も読める。
...
**status / 優先度のチェックボックス**で絞り込め、種別・エッジ種別と同じ扱いなので、
外したノードは経路としても辿られなくなり、**影響範囲の計算結果が変わる**。
優先度は「高優先 (しきい値以下) / その他 / 未設定」の 3 区分に丸めて扱う。
...
検索欄はヒットしたノードを左の一覧に絞り込み、同時に**図の上でも暈し (underlay) で示す**。
...
戻る/進むで表示状態を辿れる。ツールバーの**「リンクをコピー」**で、いまの表示状態の
...
**図 (canvas) 以外はすべてキーボードで辿れる**。フォーカスの位置は `:focus-visible`
...
- **`Goal` と `Need` を隔てる規則が語尾しか無く、自分の `Goal` 層が崩れていた。**
  ...
  G-2〜G-5 は動機づけている `Need` の言い換え (主語を落として願望形を「〜できる状態にする」に
  変えただけ) になり、G-4 に至っては「**機械が**文脈を揃える」とシステムを主語にしている。
  中間 `Goal` 層を丸ごと消して 8 つの `Need` を G-1 に直付けしても `--strict` は通る
  = この層は構造的な荷重を負っていない。一方で主語の規約 (`Need` だけが役割を書く) は
  両モデルの該当 60 ノードすべてで守られており、**規約は存在するのに文書にも検査にも無かった**。
  型の選び方は[どの型に書くか](#どの型に書くか-主語で決める)に書き下したが、
  機械の検査と G-4 の本文修正はまだ入れていない
