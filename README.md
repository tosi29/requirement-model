...
- 中央上部には**グラフ / テーブルの切替**を置き、向き・再配置・フォーカス・
...
- **フォーカス**: 起点を固定して上流・下流、または近傍だけを図に描く。別のノードを
  クリックすると detail と起点までの最短経路の強調だけが変わり、配置・倍率を保つ
...
| 項目 | 中身 | 書かれないとき |
|---|---|---|
| `node` | 選択中のノード (フォーカス中は固定した起点) | 未選択 |
| `types` / `edges` | 表示中のノード種別 / エッジ種別 (定義順) | 全種別が ON |
| `status` | 表示中の status | 全部が ON |
| `dir` | グラフの向き (`LR`) | `TD` |
| `view` | 中央ペインのタブ (`table`) | グラフ |
| `focus` | `impact` (上流＋下流) / `upstream` / `downstream` / `1`・`2`・`3` (近傍) | 切 (全体を描く) |
| `detail` | フォーカス中に詳細を表示するノード | 起点の詳細 |
...
| `Esc` | フォーカスを解除する → 選択を解除する → 検索語を消す → 入力欄から手を離す (この順) |
...
グラフ領域内のフォーカスアイコンから、表示範囲を選ぶ。

| 選択肢 | 図に描く範囲 |
|---|---|
| 上流＋下流 | 起点から有向に辿れる上流・下流と起点 |
| 上流のみ / 下流のみ | 指定した方向に辿れるノードと起点 |
| 近傍 1〜3 ホップ | 向きを無視して指定ホップ以内にあるノードと起点 |

選択中のノードが起点として固定され、その部分グラフで並べ直す。未選択なら、次に選ぶ
ノードが起点になる。上流・下流の探索には左サイドバーの深さが効き、既定は無制限。
種別・status・エッジの絞り込みも探索に効く。起点をフィルタで隠すと図は空になる。

モード中は別のノードをクリックしても**起点・配置・パン・倍率を変えず**、右側の detail を
切り替える。detail の対象を破線の太枠、その対象と起点を結ぶ最短経路 1 本のノード・エッジを
紫で強調し、他のノード・エッジは減光する。分析では経路の途中でエッジの向きを反転しない。
同長の経路では上流を優先し、各方向内ではモデルのエッジ順で 1 本を選ぶ。
背景クリックでは起点を解除しない。上部の「フォーカスを解除」、選択肢の「切」、または
`Esc` で全体表示に戻り、起点の選択は残る。

パン・ズーム・方向切替・再配置・ダウンロード・種別と status の凡例は通常のグラフと共通。
フォーカス中は起点・上流・下流・詳細と経路の凡例も表示する。
左の一覧とテーブルはフォーカスで絞らず、そこから表示範囲外のノードを選んだ場合も
起点は動かない。detail に表示範囲外である旨を示し、経路の強調は行わない。
詳細の件数・コピー本文は detail の対象と影響範囲設定に従い、図の範囲では切らない。

起点・表示範囲・detail の対象は URL に載る
(`#node=FR-120&focus=impact&detail=FR-121`) ので、同じ分析状態をリンクで渡せる。
