...
- **編集モード**では図のノードを直接編集し、接続をドラッグで追加、線を選んで削除できる。
  `req plan` と同じ semantic diff を確認・コピーし、GitHub Issue の編集画面へ渡せる
...
### 編集モードと semantic diff

ツールバーの**編集モード**を押すと、図を見たまま次の操作で変更案を作れる。

- ノードをダブルクリックし、近くに出る小さな編集欄で本文と status を変更する
- ノードを選び、右側の青い接続点から別ノードへドラッグして relation を追加する。
  接続可能なノードだけが強調され、relation の種類はメタモデル上の型から決まる
- 線を選び、表示される削除ボタンか `Delete` / `Backspace` で relation を削除する
- 外部参照、`acceptance_criteria`、`suppress` は右ペインの「詳細フィールドを編集」で
  JSON 配列として変更する

- 生成時の正規化グラフを `base`、編集中の独立コピーを `draft` としてブラウザ内に保持する
- 「変更を確認」は、Python ソースの行差分ではなく、`req plan` と同じく出所を除外した
  **ノード／フィールド単位の構造 diff** を表示する。「diff をコピー」で Clipboard に渡せる
- `--repo-url https://github.com/OWNER/REPO` を付けてサイトを生成していれば、diff の title と body を
  クエリパラメータへ埋めた **GitHub Issue の新規編集画面**を開ける。作成確定は GitHub 上で行う
- 「変更を破棄」は `draft` を `base` から作り直す。定義ファイルや `model.json` は変更しない
- ブラウザ側で担保するのはフィールド型、status 候補、relation の接続可能型、参照先の存在まで。
  Need の語尾などを含む完全な検証は、diff を定義ファイルへ反映した後の `req validate` を正とする

編集モードも生成済み HTML 内の JavaScript だけで動き、バックエンドや GitHub API を使わない。
自由配置、ノード ID／型の変更、ノード追加・削除、draft の永続化、定義ファイルへの書き戻しは
対象外である。
