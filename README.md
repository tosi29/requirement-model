...
渡さなければ今まで通りただの文字列として出る。組み立てはページ側
(`site_logic.js` の `sourceUrl()`) で行い、埋め込むのは URL と参照だけ。
出所が絶対パスのとき (リポジトリ内の位置が決まらないとき) はリンクにしない。
...
Mermaid は `render.py` の `render_mermaid()` と同じ書式で組む。両者が一致することは
`tests/test_site_js.py` が `examples/sample.py` で突き合わせるので、「サイトから
書き出した図」と「CLI が出す図」が別物になることはない。SVG は Cytoscape の描画の
**近似**である (位置・大きさ・ラベル・配色は実測どおり。二重線や角の落ちなど、
SVG に無い表現だけを近い形に置き換える)。
...
配色は既定で OS 設定に従う。明るい部屋やプロジェクタでは追従されると困るので、
ヘッダのボタンで **自動 → 明 → 暗** と切り替えて固定できる。固定した選択は
`localStorage` に残り、次に開いたときも効く (`<html data-theme="light|dark">` が
付き、自動に戻すと外れる)。図の配色も同じ CSS 変数から引き直す。

**絞り込みと表示 (種別・status・優先度・エッジ種別・向き・タブ・並び順) も
`localStorage` に残る**。パーマリンク ([前節](#url-に載る表示状態-パーマリンク)) は
「いまの画面を人に渡す」ためのもので、次に自分が開いたときには効かないためである。
