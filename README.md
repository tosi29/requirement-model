...
- 中央上部には**グラフ / フォーカス分析 / テーブルの切替**を置き、向き・再配置・フォーカス・
  ズーム・等倍・全体表示・図のダウンロードは**グラフ領域右下のアイコン操作**にまとめる。
  テーブル表示中は隠す
- **status を枠線の線種で表示**する
...
- **フォーカス分析**: ノードを選んで専用タブへ入ると、起点に影響するノードとエッジだけを
  表示する。別のノードをクリックしても起点は固定し、detail と接続経路の強調だけを変える
- 選択・絞り込み・表示中のタブが **URL に載る**。URL を渡せば相手にも同じ画面が出る
- 配色は OS 設定に従い、ヘッダのボタンで**明 / 暗に固定**もできる。絞り込みは
  次に開いたときに戻る
- **SVG のノードを含むすべての操作要素をキーボードだけで辿れる**。グラフ領域内のアイコン操作も
  フォーカスの位置は `:focus-visible` の輪郭で常に見える (マウス操作では出ない)
...
凡例には実際に効いているスタイルがそのまま出て、種別 (色) と status (線種) は別の段に分かれる。左サイドバーの
...
- 通常表示・近傍表示では深さ・向きを変えても**再レイアウトは走らない**。
  フォーカス分析では、深さが描画範囲にも効くため、変更時に並べ直す
- 絞り込み (種別・status・エッジ種別) とは重ねて効く。外したノードは
  経路として辿られないので、深さの数え方もそのぶん変わる

フォーカスの描画範囲では、詳細ペインの件数やコピー本文を切らない。
フォーカス分析では有向探索に固定し、「向きを無視して辿る」は無効になる。
解除すると元の向き設定に戻る。
...
| 項目 | 中身 | 書かれないとき |
|---|---|---|
| `node` | 選択中のノード (フォーカス分析中は固定した起点) | 未選択 |
| `types` / `edges` | 表示中のノード種別 / エッジ種別 (定義順) | 全種別が ON |
| `status` | 表示中の status | 全部が ON |
| `dir` | グラフの向き (`LR`) | `TD` |
| `view` | 中央ペインのタブ (`analysis` / `table`) | グラフ |
| `focus` | 通常グラフの近傍表示 (`1` / `2` / `3` ホップ) | 切 |
| `detail` | フォーカス分析中に詳細を表示するノード | 起点の詳細 |
...
(起点・detail の対象・検索語は持ち越さない)。渡された URL のハッシュが最優先なので、
パーマリンクに相手の前回の絞り込みが混ざることはない。

### キーボード操作

**SVG のノードを含むすべての操作要素をキーボードで辿れる**。グラフ領域内のアイコン操作も
フォーカスの位置は `:focus-visible` の輪郭で常に見える (マウス操作では出ない)。
...
| `Esc` | フォーカス分析からグラフへ戻る → 選択を解除する → 検索語を消す → 入力欄から手を離す (この順) |
...
絞り込んだ図をそのまま PR や資料に持っていけるよう、グラフ領域内から
**いま見えているぶんだけ**を書き出せる。操作はテーブル表示では隠れる。

グラフ領域内のダウンロードアイコンを押し、表示されたアイコンから形式を選ぶ。
...
通常のグラフでは、右下のフォーカスアイコンから「近傍 1〜3 ホップ」を選ぶと、
選択ノードの周囲だけを描く。これは向きを無視する近傍表示で、選択すると起点も移る。

### 影響するノードだけを見る (フォーカス分析)

1. グラフまたはテーブルで、分析の起点にするノードを選ぶ。
2. 中央上部の **「フォーカス分析」タブ**を選ぶ。
3. 表示されたノードをクリックし、右側の detail と起点までの経路を確認する。

起点未選択では分析タブは無効になる。分析に入ると、既定で起点とその上流・下流だけを
描く。影響しないノード・エッジは減光で残さず、図から除外する。
分析図では要求グループの枠を使わず、「上流」「下流」の枠に分け、間に起点を置く。
縦表示では上から、横表示では左から「上流 → 起点 → 下流」の順に並ぶ。

上流と下流は常に両方を表示する。左サイドバーの深さ (既定は無制限)、
種別・status・エッジの絞り込みが探索に効く。
分析は有向に固定し、経路の途中でエッジの向きを反転しない。通常グラフの近傍表示の
ホップ数は分析には効かない。

**図内の**別のノードをクリックしても**起点・表示範囲・配置・パン・倍率は変わらない**。
detail の対象を破線の太枠、起点を結ぶ接続するすべての経路のノード・エッジを紫で強調する。
経路外でも起点の影響範囲に含まれるノードは図に残る。直結する経路と別のノードを経由する経路がある場合は、両方を強調する。

左のノード一覧・検索・指摘は通常画面と同じ範囲を保ち、そこから選ぶと**分析の起点が切り替わる**。
図内のクリックによる detail の変更と、一覧からの起点変更を使い分けられる。

パン・ズーム・方向切替・再配置・書き出し・種別と status の凡例はグラフ画面と共通。
専用の戻るボタン・起点IDと件数の表示・上下流の切替・専用凡例は置かない。
Mermaid と SVG の書き出しも
分析範囲に限定する。詳細の件数と LLM 向けコピー本文は、detail の対象と影響範囲設定
から計算し、分析の描画範囲では切らない。

背景クリックでは起点を解除しない。グラフタブまたは `Esc` で
通常グラフへ戻り、起点の選択は残る。フィルタで起点を隠した場合は分析図が空になり、
全体表示へは戻さない。URL の detail が分析範囲外なら起点の詳細に戻す。

起点・分析タブ・detail の対象は URL に載る
(`#node=FR-120&view=analysis&detail=FR-121`) ので、同じ分析状態をリンクで渡せる。
...
- **外部の課題管理も `Reference` で辿れる。** 未着手の FR に対応する GitHub Issue は
  `Reference` として `source` に保持する。外部情報を要求グラフのノードにせず、
  URL を含む値として各要求から機械的に辿れる
