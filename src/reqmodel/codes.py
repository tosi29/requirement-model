...
CHECK_CODES: dict[str, CheckCode] = {
    ...
    for entry in (
        ...
        _code("structure.conflict_unresolved", 2, "未解消の conflict", True),
        _code("structure.resolve_no_conflict", 2, "宣言されていない対立の解消", True),
        _code("structure.missing_source", 2, "源泉 (has_source) が無い", True),
        _code("structure.missing_acceptance_criteria", 2, "受け入れ基準が無い", True),
