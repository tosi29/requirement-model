...
test("bandedLayout は RequirementGroup を最大幅で複数行に折り返す", () => {
  ...
  const placed = bands.flatMap((band, group) =>
    band.members.map((id, node) =>
      placedNode(id, "FunctionalRequirement", group * 400 + node * 70, 0),
    ),
  );
  ...
  const rows = new Set([...frames.values()].map((frame) => frame.y));
  assert.ok(rows.size > 1, "Requirements 段内でグループ行が折り返される");
  for (const frame of frames.values()) assert.ok(frame.w <= 300 + 28);
});

test("bandedLayout は単一 RequirementGroup 内の多数ノードを複数行に折り返す", () => {
  const band = {
    key: "group:many",
    label: "多数",
    members: Array.from({ length: 12 }, (_, index) => `FR-${index}`),
  };
  const placed = band.members.map((id, index) =>
    placedNode(id, "FunctionalRequirement", index * 100, 0),
  );
  const { positions, frames } = bandedLayout([band], placed, [], "TD", {
    groupMaxWidth: 250,
  });

  const rows = new Set([...positions.values()].map((position) => position.y));
  assert.ok(rows.size > 1, "グループ内部のノード行が折り返される");
  assert.ok(frames.get("group:many").w <= 250 + 28);
});

test("bandedLayout は RequirementGroup の LR でも枠とノードを重ねない", () => {
  const bands = [
    { key: "group:a", label: "A", members: ["FR-1", "FR-2", "FR-3"] },
    { key: "group:b", label: "B", members: ["QR-1", "QR-2", "QR-3"] },
  ];
  const placed = [
    placedNode("FR-1", "FunctionalRequirement", 0, 0),
    placedNode("FR-2", "FunctionalRequirement", 0, 10),
    placedNode("FR-3", "FunctionalRequirement", 0, 20),
    placedNode("QR-1", "QualityRequirement", 0, 400),
    placedNode("QR-2", "QualityRequirement", 0, 410),
    placedNode("QR-3", "QualityRequirement", 0, 420),
  ];
  const { positions, frames } = bandedLayout(bands, placed, [], "LR", {
    requirementsMaxWidth: 120,
    groupMaxWidth: 90,
  });

  assert.ok(
    frames.get("group:a").x < frames.get("group:b").x,
    "LR では次のグループ行が右へ送られる",
  );
  for (const [id, position] of positions) {
    const frame = frames.get(id.startsWith("FR") ? "group:a" : "group:b");
    assert.ok(position.x > frame.x - frame.w / 2 && position.x < frame.x + frame.w / 2);
    assert.ok(position.y > frame.y - frame.h / 2 && position.y < frame.y + frame.h / 2);
  }
});
